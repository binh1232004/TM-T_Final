...
export const getCatalogs = (name = null) => {
    ...
    return get(ref(database, "/products/catalogs")).then((snapshot) => {
        if (snapshot.exists()) {
            ...
            else return Object.keys(result).filter((key) => result[key].name === name).reduce((obj, key) => {
                obj[key] = result[key];
                return obj;
            }, {});
        } else {
...
export const getProducts = (catalog, limit = 10, sort = -1) => {
    ...
    return getCatalog(catalog).then((result) => {
        if (result) catalog = Object.keys(result)[0];
        get(ref(database, `/products/items/${catalog}`)).then((snapshot) => {
            if (snapshot.exists()) {
...
export const getProduct = (id, catalog = null) => {
    ...
    return getCatalog(catalog).then((result) => {
        if (result) catalog = Object.keys(result)[0];
        get(ref(database, `/products/items/${catalog}/${id}`)).then((snapshot) => {
            if (snapshot.exists()) {
                return snapshot.val();
            } else {
                return null;
