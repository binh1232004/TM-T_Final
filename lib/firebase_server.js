...
export const getProducts = (catalog, limit = 10, sort = -1) => {
    ...
    return getCatalogs(catalog).then((result) => {
...
export const searchProducts = (query, limit = 10) => {
    return getCatalogs().then((catalogs) => {
        let result = {};
        return Promise.all(Object.keys(catalogs).map((catalog) => {
            return getProducts(catalog, -1).then((products) => {
